...
def train_step(
  ...
  new_params = updater.post_gradient_update(new_params, new_opt_state)  # pytype: disable=wrong-arg-types

  training_logs['l2_grads'] = jnp.sqrt(
      sum([jnp.vdot(g, g) for g in jax.tree_util.tree_leaves(grad)])
  )
  logging.info('Summarizing sparsity during training')
  training_logs.update(
      utils.summarize_sparsity(new_params, only_total_sparsity=True)
  )
  training_logs['_total_sparsity_after_pre_forward'] = utils.summarize_sparsity(
      updated_params, only_total_sparsity=True
  )['_total_sparsity']
